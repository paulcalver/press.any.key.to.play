class Shape {
  ...
  update() {
    ...
    // No velocity limiting - allow unlimited speed
  ...
  // Behavior: Orbit around a target position
  orbitTarget(target, orbitStrength = 0.15, pullStrength = 0.05) {
    let toCenter = p5.Vector.sub(target, this.position);
    
    // Create perpendicular force for orbit
    let orbit = createVector(-toCenter.y, toCenter.x);
    orbit.setMag(orbitStrength);
    
    // Add slight centripetal force to maintain orbit
  ...
  // Behavior: Move away from a target position
  fleeTarget(target, strength = 0.12) {
    let push = p5.Vector.sub(this.position, target);
    push.setMag(strength);
    this.velocity.add(push);
  }

  // Static helper: Calculate center of mass for a group of shapes
  static getCenterOfMass(shapes) {
    if (shapes.length === 0) return createVector(0, 0);
